...
class Encoder(nn.Module):
    ...
    def forward(self, src):
        ...
        #  when the org_imgs is a pad token are all zeros
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, trg, teacher_forcing_ratio = 0.5):
        ...
        #encoder_outputs is all hidden states of the org_imgs sequence, back and forwards
        #hidden is the final forward and backward hidden states, passed through a linear layer
        encoder_outputs, hidden = self.encoder(src)
                
        #first org_imgs to the decoder is the <sos> tokens
        input = trg[0,:]
...
#         mask = self.create_mask(src)
        ...
        for t in range(1, trg_len):
            ...
            #insert org_imgs token embedding, previous hidden state, all encoder hidden states
            #  and mask
...
#            teacher_force = random.random() < teacher_forcing_ratio
            ...
            #if teacher forcing, use actual next token as next org_imgs
