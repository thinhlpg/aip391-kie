...
from config import inference_cls_out_viz_dir, inference_cls_out_txt_dir, cls_visualize, gpu, cls_ocr_thres, craft_rot_out_txt_dir,\
    gpu, inference_rotated_img_dir, inference_rotated_boxes_txt_dir, inference_cls_out_tsv_dir

os.environ["PYTHONIOENCODING"] = "utf-8"
pred_time = datetime.today().strftime('%Y-%m-%d_%H-%M')

img_dir = inference_rotated_img_dir
img_path = ''
anno_dir = inference_rotated_boxes_txt_dir
anno_path = ''
...
def main():
    ...
    check_point_txt_list = get_text_file_list(inference_cls_out_txt_dir)
    ...
    for idx, img_name in enumerate(list_img_path):
        ...
        output_tsv_path = os.path.join(inference_cls_out_tsv_dir, os.path.basename(img_name).split('.')[0] + '.tsv')
        output_txt_path = os.path.join(inference_cls_out_txt_dir, os.path.basename(img_name).split('.')[0] + '.txt')
        output_viz_path = os.path.join(inference_cls_out_viz_dir, os.path.basename(img_name))
        if write_file:
            write_output_tsv(boxes_list, final_values, final_probs, output_tsv_path, prob_thres=cls_ocr_thres)
            write_output_txt(boxes_list, final_values, final_probs, output_txt_path, prob_thres=cls_ocr_thres)
...
def write_output_tsv(list_boxes, values, probs, result_file_path, prob_thres=0.7):
    result = ''
    for idx, box in enumerate(list_boxes):
        s = [str(i) for i in box]
        if probs[idx] > prob_thres:
            line = str(idx + 1) + ',' + ','.join(s) + ',' + values[idx]
        else:
            line = str(idx + 1) + ',' + ','.join(s) + ','
...
def write_output_txt(list_boxes, values, probs, result_file_path, prob_thres=0.7):
    result = ''
    for idx, box in enumerate(list_boxes):
        s = [str(i) for i in box]
        if probs[idx] > prob_thres:
