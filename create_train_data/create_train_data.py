...
import numpy as np
from myutils.common import IoU, cer_loss_one_image, type_map, \
    color_map,get_list_gt_poly, get_list_icdar_poly, get_file_list, euclidean_distance
from myutils.spell_check import validate_SELLER, validate_TOTAL_COST_amount, \
...
from config import (
    ...
    new_kie_out_txt_dir,
    new_kie_inp_img_dir,
    reversed_filtered_csv,
    kie_out_txt_dir,
    rot_out_reversed_filtered_dir,
    filtered_csv,
    cls_out_txt_dir,
    json_data_path,
    kie_boxes_transcripts,
    combined_kie_out_txt_dir,
    combined_kie_train_images, kie_train_dir)
...
def parse_anno_from_csv_to_icdar_result(csv_file, icdar_dir, output_dir, img_dir=None, debug=False):
    with open(csv_file, encoding='utf-8') as csv_file:
        ...
        for n, row in enumerate(csv_reader):
            ...
            if os.path.exists(output_icdar_path):
...
def modify_kie_training_data_by_rules(txt_dir, json_data_path, debug=False):
    list_files = get_file_list(txt_dir, ext=['.txt'])
...
def create_data_pick_boxes_and_transcripts(icdar_dir, output_dir):
    list_file = get_file_list(icdar_dir, ext=['txt'])
...
def create_data_pick_csv_train_val(train_dir, train_ratio=0.92):
    list_files = get_file_list(os.path.join(train_dir, 'images'))
...
if __name__ == '__main__':
    ...
    # parse_anno_from_csv_to_icdar_result(csv_file=reversed_filtered_csv,
    #                                     icdar_dir=cls_out_txt_dir,
    #                                     output_dir=kie_out_txt_dir,
    ...
    # from myutils.visualize import viz_icdar_multi
    # viz_icdar_multi(img_dir=new_kie_inp_img_dir,
    #                 anno_dir=new_kie_out_txt_dir,
    #                 save_viz_dir=new_kie_out_viz_dir,
    ...
    #
    create_data_pick_boxes_and_transcripts(icdar_dir=combined_kie_out_txt_dir,
                                           output_dir=kie_boxes_transcripts)


    # create_data_pick_csv_train_val(kie_train_dir, train_ratio=0.8)
