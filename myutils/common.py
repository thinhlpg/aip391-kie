...
def get_text_file_list(dir, ext=['txt', 'TXT']):
...
def get_file_list(dir, ext=['jpg', 'png', 'JPG', 'PNG']):
    included_extensions = ext
    file_names = [fn for fn in os.listdir(dir)
                  if any(fn.endswith(ext) for ext in included_extensions)]
    return file_names
...
def get_img_file_list_recursively(folder, ext=['jpg', 'png', 'JPG', 'PNG']):
    file_names = []
...
def get_list_gt_poly(row, add_key_to_value=False):
    ...
    for idx, box in enumerate(boxes):
        ...
        total_box = 0
        for coor in coors:
            if len(coor) < 8:  # lỗi cho này do csv lúc xoay không có cấu trúc giống csv gốc
                continue
            else:
...
def filter_data(src_dir, dst_dir, dst_anno=None):  #filter data in dst_dir by src_dir
    list_files = get_file_list(src_dir)
    list_files2=get_file_list(dst_dir)
