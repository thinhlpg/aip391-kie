...
from myutils.common import poly, get_file_list, get_list_gt_poly, type_map
...
def viz_icdar_multi(img_dir, anno_dir, save_viz_dir, extract_kie_type=False, ignor_type=[1]):
    list_files = get_file_list(img_dir)
    checkpoint_viz_list = get_file_list(save_viz_dir)
    for idx, file in enumerate(list_files):
        if file in checkpoint_viz_list:
            print('Skipping', file)
            continue
        if idx < 0:
            continue
...
def viz_output_of_pick(img_dir, output_txt_dir, output_viz_dir):
    list_output_txt = get_file_list(output_txt_dir, ext=['txt'])
