...
keywords_TOTAL_COST = ['tổng tiền', 'cộng tiền hàng',
                       'tổng cộng', 'thanh toán', 'tại quầy']
...
def filter_training_data_by_rules(csv_file, output_csv_file, output_filtered_dir, img_dir=None):
    with open(csv_file) as csv_file:
        ...
        for n, row in enumerate(csv_reader):
            ...
            num_keys = {'SELLER': 0, 'ADDRESS': 0,
                        'TIMESTAMP': 0, 'TOTAL_COST': 0}
            ...
            if filter_too_many_keys:
                if num_keys['TOTAL_COST'] > 4:
                    ...
                    print(total_many_few_key, img_name,
                          'filter_training_data_by_rules. Too many TOTAL_COST')
                    ignore_file = True
                    # if img_dir is not None:
                    #     img = cv2.imread(os.path.join(img_dir, img_name))
                    #     cv2.imshow('wrong anno', img)
            ...
            if filter_too_few_keys:
                ...
                if total_exist_keys < 3:
                    ...
                    ignore_file = True
                    print(total_many_few_key, img_name,
                          'filter_training_data_by_rules. Very few keys in img')
                    # if img_dir is not None:
                    #     img = cv2.imread(os.path.join(img_dir, img_name))
                    #     cv2.imshow('wrong anno', img)
                    #     cv2.waitKey(0)
            if not ignore_file:
                ...
                shutil.copy(os.path.join(img_dir, img_name),
                            os.path.join(output_filtered_dir, img_name))
            else:
...
if __name__ == '__main__':
    from config import raw_train_img_dir, raw_csv, filtered_train_img_dir, filtered_csv
