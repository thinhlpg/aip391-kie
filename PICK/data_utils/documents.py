...
class Document:
    def __init__(self, boxes_and_transcripts_file: Path, image_file: Path,
        ...
        try:
            ...
            if self.training:
                # assign iob label to org_imgs text though exactly match way, this process needs entity-level label
...
def preprocess_transcripts(transcripts: List[str]):
    '''
    rotation_and_filter texts into separated word-level list, this is helpful to matching tagging label between source and target label,
